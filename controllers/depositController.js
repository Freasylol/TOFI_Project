...
class DepositController {
    ...
    async createOne(req, res) {
        ...
        const deposit = await Deposit.create({sum, date, term, percent, bankAccountId});
    ...
    async updateOne(req, res) {
        const id = Number(req.params.id);
        const {sum, date, term, percent, bankAccountId} = req.body;
        const deposit = await Deposit.update({sum, date, term, percent, bankAccountId}, {where: {id: id}});
        return res.json(deposit);
    }
